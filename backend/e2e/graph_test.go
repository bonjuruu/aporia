...
func TestGraphJourney(t *testing.T) {
	...
	doRequest(t, http.MethodGet, "/api/graph", nil, token, http.StatusOK, &graphData)
	...
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/graph/subgraph/%s", republicNode.ID), nil, token, http.StatusOK, &subgraphData)
	assert.Len(t, subgraphData.Edges, 1)

	// Path from Plato to Logic should go through Aristotle
	var pathData response.GraphData
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/graph/path?from=%s&to=%s", platoNode.ID, logicNode.ID), nil, token, http.StatusOK, &pathData)
	...
	doRequest(t, http.MethodGet, "/api/search?q=logic", nil, token, http.StatusOK, &searchResult)
	...
	doRequest(t, http.MethodGet, "/api/annotations", nil, token, http.StatusOK, &annotationList)
