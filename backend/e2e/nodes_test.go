...
func TestNodesJourney(t *testing.T) {
	...
	doRequest(t, http.MethodGet, "/api/nodes", nil, auth, http.StatusOK, &emptyNodeList)
	...
	}, auth, http.StatusCreated, &conceptNode)
	...
	doRequest(t, http.MethodGet, "/api/nodes", nil, auth, http.StatusOK, &nodeList)
	assert.Len(t, nodeList, 4)

	// Get Plato by ID and verify properties
	var detail response.NodeDetail
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/nodes/%s", platoNode.ID), nil, auth, http.StatusOK, &detail)
	...
	// Delete Plato
	doRequest(t, http.MethodDelete, fmt.Sprintf("/api/nodes/%s", platoNode.ID), nil, auth, http.StatusOK, nil)

	// Verify it's gone
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/nodes/%s", platoNode.ID), nil, auth, http.StatusNotFound, nil)
	...
	doRequest(t, http.MethodGet, "/api/search?q=forms", nil, auth, http.StatusOK, &searchResult)
