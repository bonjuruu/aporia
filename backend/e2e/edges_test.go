...
func TestEdgesJourney(t *testing.T) {
	...
	}, token, http.StatusCreated, &createdEdge)
	...
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/nodes/%s", platoNode.ID), nil, token, http.StatusOK, &platoDetail)
	...
	doRequest(t, http.MethodGet, fmt.Sprintf("/api/nodes/%s", aristotleNode.ID), nil, token, http.StatusOK, &aristotleDetail)
	...
	// Delete the edge
	doRequest(t, http.MethodDelete, fmt.Sprintf("/api/edges/%s", createdEdge.ID), nil, token, http.StatusOK, nil)
